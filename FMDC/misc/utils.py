...
from model.optical_deformable_module import optical_flow_warping
...
# +
def save_results_color(iter, exp_path, restore, img0, img1, mask):  # , flow):
...
def save_warp(iter, exp_path, restore, img0_warp, img1_warp):
    UNIT_H , UNIT_W = img0_warp.size(2), img0_warp.size(3)
    pil_input0 = restore(img0_warp[0])
    pil_input1 = restore(img1_warp[0])
    pil_input0 = np.array(pil_input0)
    pil_input1 = np.array(pil_input1)
    pil_input0 = Image.fromarray(cv2.cvtColor(pil_input0, cv2.COLOR_LAB2RGB))
    pil_input1 = Image.fromarray(cv2.cvtColor(pil_input1, cv2.COLOR_LAB2RGB))
    imgs = [pil_input0,pil_input1]
    w_num , h_num=1,2
    target_shape = (w_num * (UNIT_W + 10), h_num * (UNIT_H + 10))
    target = Image.new('RGB', target_shape)
    count = 0
    ...
    target.save(os.path.join(exp_path,'noof','{}_warp.jpg'.format(iter)))
def save_results_mask(iter, exp_path, restore, img0, img1, pred_map0, gt_map0,\
                       pred_map1, gt_map1, pred_mask_out, gt_mask_out, pred_mask_in, gt_mask_in, f_flow,b_flow): 
    ...
    for idx, tensor in enumerate(zip(img0.cpu().data, img1.cpu().data,pred_map0, gt_map0, pred_map1, gt_map1, \
                                     pred_mask_out, gt_mask_out, pred_mask_in, gt_mask_in)):
        if idx > 1:  # show only one group
            break
        pil_input0 = restore(tensor[0])
        pil_input1 = restore(tensor[1])
        ...
        imgs = [pil_input0, pil_gt0, pil_output0, pil_f_flow,pil_gtmaskout, pil_maskout, \
                pil_input1, pil_gt1, pil_output1, pil_b_blow,pil_gtmaskin, pil_maskin]
