...
parser.add_argument(
    '--model_path', type=str, default='./exp/SENSE/03-16_17-12_SENSE_VGG16_FPN_5e-05/ep_6_iter_85500_mae_3.893_mse_7.688_seq_MAE_11.661_WRAE_20.350_MIAE_2.173_MOAE_2.477.pth',
...
#                 if frame_signal == 'match' or not opt.skip_flag:
                ...
                if frame_signal == 'match':
                    pred_map, gt_den,_,_, pre_inflow, gt_inflow, pre_outflow, gt_outflow = net.test_or_validate(img,img_rgb, target)
                    ...
                    gt_dict['inflow'].append(torch.tensor(gt_inflow))
                    gt_dict['outflow'].append(torch.tensor(gt_outflow))
                    ...
                    print('den_gt: %.2f den_pre: %.2f gt_crowd_flow: %.2f, pre_crowd_flow: %.2f gt_inflow: %.2f pre_inflow:%.2f'
                          % (gt_count, pred_cnt, gt_crowdflow_cnt, pre_crowdflow_cnt,gt_inflow,
                             pre_inflow))
...
#         MAE, MSE, WRAE, MIAE, MOAE, cnt_result = (s_pred_dict, s_gt_dict, intervals)
